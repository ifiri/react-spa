 ...
 */
class ParticipantsList extends EntriesfulComponent {
    state = {
        value: null,
    };
    ...
    getListOptions() {
        ...
        for(const index in entries) {
            ...
            if(entry) {
                const User = new UserEntity(entry);

                if(entry.id) {
                    if(value && value === entry.id) {
                        options.push((
                            <option key={key} value={entry.id} selected>{User.getFullname()}</option>
                        ));
                    } else {
                        options.push((
                            <option key={key} value={entry.id}>{User.getFullname()}</option>
                        ));
                    }
                }
            }
...
export default connect(mapStateToProps, mapDispatchToProps)(withMatch(ParticipantsList));