...
class Select extends EntriesfulComponent {
    ...
    render() {
        ...
        const { value = null, label = null, variant = 'standard', icon = null } = this.props;

        return <FormControl variant="filled">
            {(() => {
                if(label) {
                    return <InputLabel htmlFor="filled-age-native-simple">Age</InputLabel>;
            ...
            {(() => {
              ...
              <MUISelect
                ...
                variant={variant}
                input={<FilledInput name="age" name={attributes.name} />}
                IconComponent={(() => {
                    return <SvgIcon className="select__caret" component={svgProps => (
                        <svg {...svgProps}>
                            <use xlinkHref="#filter-caret" />
                        </svg>
                        )}
                    ></SvgIcon>;
                })}
     ...
     */
    getListOptions() {
        const { items = null, value = null, placeholder = 'Выберите...' } = this.props;
